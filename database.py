...
def calculate_block_hash(prev_hash, timestamp, action, performed_by, payload_hash):
    """Calculates cryptographic SHA-256 block hash linking chain entries."""
    raw = f"{prev_hash}|{timestamp}|{action}|{performed_by}|{payload_hash}"
...
def init_db():
    ...
    # Chain of Custody Ledger with Merkle/Hash Chain
    cursor.execute("""
    ...
    # Activity Logs / Tamper-Evident Audit Trail
    cursor.execute("""
...
def log_activity(action, user="Investigator", details=""):
    ...
    prev_hash = last_row[0] if last_row and last_row[0] else "GENESIS_AUDIT_BLOCK_000000000000"
    block_hash = calculate_block_hash(prev_hash, now, action, user, details)

    cursor.execute("""
        INSERT INTO audit_logs (timestamp, action, user, details, prev_hash, block_hash)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (now, action, user, details, prev_hash, block_hash))
    conn.commit()
    conn.close()
...
def record_chain_of_custody(evidence_id, action, performed_by, verification_hash, notes=""):
    ...
    # Fetch last block hash for this evidence
    cursor.execute("SELECT block_hash FROM chain_of_custody WHERE evidence_id = ? ORDER BY id DESC LIMIT 1", (evidence_id,))
    last_row = cursor.fetchone()
    prev_hash = last_row[0] if last_row and last_row[0] else "GENESIS_CUSTODY_BLOCK_0000000000"
    block_hash = calculate_block_hash(prev_hash, now, action, performed_by, verification_hash)

    cursor.execute("""
        INSERT INTO chain_of_custody (evidence_id, action, performed_by, timestamp, verification_hash, prev_block_hash, block_hash, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (evidence_id, action, performed_by, now, verification_hash, prev_hash, block_hash, notes))
    conn.commit()
    conn.close()

def verify_chain_integrity(evidence_id):
    """
    Cryptographically verifies the Merkle/Hash Chain for an evidence file
    to prove zero tampering.
    """
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM chain_of_custody WHERE evidence_id = ? ORDER BY id ASC", (evidence_id,))
    rows = cursor.fetchall()
    conn.close()

    if not rows:
        return {"valid": True, "count": 0, "message": "No custody records"}

    prev_expected = "GENESIS_CUSTODY_BLOCK_0000000000"
    for row in rows:
        if row["prev_block_hash"] != prev_expected:
            return {"valid": False, "broken_id": row["id"], "message": f"Hash chain broken at block #{row['id']}"}
        recomputed = calculate_block_hash(row["prev_block_hash"], row["timestamp"], row["action"], row["performed_by"], row["verification_hash"])
        if recomputed != row["block_hash"]:
            return {"valid": False, "broken_id": row["id"], "message": f"Block hash mismatch at block #{row['id']}"}
        prev_expected = row["block_hash"]

    return {"valid": True, "count": len(rows), "message": "All cryptographic blocks verified intact."}
...
def save_investigation_record(evidence_data, analysis_results, case_id=1, investigator="Investigator"):
    ...
    # 2. Add Chain of Custody Initial Entry (Genesis Block)
    prev_hash = "GENESIS_CUSTODY_BLOCK_0000000000"
    action1 = "Evidence Acquisition & Cryptographic Hashing"
    block_hash1 = calculate_block_hash(prev_hash, now, action1, investigator, evidence_data["sha256"])
    cursor.execute("""
        INSERT INTO chain_of_custody (evidence_id, action, performed_by, timestamp, verification_hash, prev_block_hash, block_hash, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (
    ...
    # 4. Add Chain of Custody Analysis Entry (Block #2)
    action2 = "AI Forensic Triage & Anomaly Analysis Complete"
    block_hash2 = calculate_block_hash(block_hash1, now, action2, "Cyber Triage AI Engine", evidence_data["sha256"])
    cursor.execute("""
        INSERT INTO chain_of_custody (evidence_id, action, performed_by, timestamp, verification_hash, prev_block_hash, block_hash, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (
...
def get_investigation_details(investigation_id):
    ...
    if inv.get("evidence_id"):
        ...
        inv["custody_integrity"] = verify_chain_integrity(inv["evidence_id"])
    else:
        inv["chain_of_custody"] = []
        inv["custody_integrity"] = {"valid": True, "count": 0}
